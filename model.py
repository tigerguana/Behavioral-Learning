...
def generator(samples, batch_size=32):
	...
	# Generate the required images and measuremnts for training 'samples' is a list of pairs/
	# ('imagePath', 'measurement')
	...
	while 1: # Loop forever so the generator never terminates
		...
		for offset in range(0, num_samples, batch_size):
			...
			for imagePath, measurement in batch_samples:
				image = cv2.imread(imagePath) #cv2.cvtColor(originalImage, cv2.COLOR_BGR2RGB)
				images.append(image)
...
centerPaths, leftPaths, rightPaths, measurements = findImages('Record/Backup')
...
h_object = model.fit_generator(train_generator, samples_per_epoch= \
				len(train_samples), validation_data=validation_generator, \
				nb_val_samples=len(validation_samples), nb_epoch=3, verbose=1)

model.save('model.h5')

#print(h_object.history.keys())
#print('Loss')
#print(h_object.history['loss'])
#print('Validation Loss')
#print(h_object.history['val_loss'])
